...
/*   Updated: 2020/08/11 16:28:14 by mdavid           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
	...
	if (cw)
		if (flag == 1)
			ft_printf("Player %d (%s) is said to be alive\n", \
				((t_process*)(ptr))->registers[0], ((t_process*)(ptr))->champ->name);
	return (flag);
}
...
int		vprint_op(t_cw *cw, void *ptr, int flag)
{
	...
	{
		ft_printf("P\t%d | %s %sOK\n", ((t_process*)(ptr))->id, \
			op_tab[((t_process*)(ptr))->opcode - 1].name, args_to_str(cw, \
				((t_process*)(ptr))));
		return (flag);
	}
	else
...
int		vprint_pcmv(t_cw *cw, void *ptr, int flag)
{
	...
	widht = widht + ((op_tab[((t_process*)(ptr))->opcode - 1].encod == 0) ? 1 : 2);
	...
	while (++i < widht)
	{
		if (((int)cw->arena[((t_process*)(ptr))->i + i] & 255) < 16)
			ft_printf("0%s", ft_itoa_base2((int)cw->arena[((t_process*)(ptr))->i \
