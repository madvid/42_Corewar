...
t_visu		init_id(t_visu *v, t_parse *p)
{
	...
	while (i < p->nb_champ)
	{
		...
		v->chp_vn[i] = SDL_CreateTextureFromSurface(v->renderer, v->chp_name[i]);
		if (!v->chp_vn[i])
		...
		v->chp_cs[i].w = 400;
		v->chp_cs[i].h = 150;
        v->chp_size[i] = TTF_RenderText_Blended(v->font_p, ft_itoa(((t_champ*)(xplr->cnt))->l_bytecode), v->color_chp[i]);
		if (!v->chp_size[i])
			printf("Error creating text : %s\n", SDL_GetError());
		v->chp_vs[i] = SDL_CreateTextureFromSurface(v->renderer, v->chp_size[i]);
		if (!v->chp_vs[i])
			printf("Error creating texture : %s\n", SDL_GetError());
        i++;
        xplr = p->lst_champs;
